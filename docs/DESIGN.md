...
### Механика Dala (Void & Scale)
*   **Void Layout**: Полный отказ от карточек, фоновых плашек, рамок и теней. Текст и контент свободно «парят» на чистом холсте (белом или черном). Пространство — это главный инструмент дизайна.
*   **Scale over Weight**: Иерархия строится за счет радикального контраста размеров. Заголовки — гигантские (до 100px+) с отрицательным трекингом (`-0.04em`), но нормальной жирности. Основной текст — крупный (18px), но экстремально тонкий (Light / 200-300 weight), чтобы казаться воздушным и неагрессивным.
*   **Форма частиц**: Не обычные точки, а крошечные контурные **треугольники** (1-2px outline). Формируют как плотные органические объекты (Матричные таблетки), так и рассеянную «ambient» пыль для глубины.
...
| **Mode** | Dark Void Theme (Pure Black `#000000` + Bone White `#FFFFFF`) |
...
:root {
  ...
  /* Параметры текста */
  --tracking-hero: -0.04em;   /* Агрессивный отрицательный трекинг для огромных заголовков */
  --tracking-h1:   -0.03em;
  --tracking-ui:   0.05em;    /* tracking для монопространственных лейблов */
  --leading-tight: 1.1;       /* Для hero и H1 */
  --leading-body:  1.5;       /* Для основного текста */
  --font-weight-body: 300;    /* Экстра-тонкий для воздушности (Dala mechanic) */
}
```
...
|---|---|---|---|---|
| Имя-заголовок (Hero) | `--font-display` | `--text-hero` | 400 | `letter-spacing: -0.04em` |
| Заголовок секции | `--font-display` | `--text-h1` | 400 | `text-transform: uppercase`, `letter-spacing: -0.03em` |
| Подзаголовок / роль | `--font-display` | `--text-h3` | 400 | `letter-spacing: 0.1em` UPPERCASE |
| Тело текста | `--font-body` | `--text-body` | 300 | `line-height: 1.5`, ультра-лёгкий вес |
...
| Индекс | ID | Название | Частицы Three.js (Dala Triangles) | Ключевой UI |
|---|---|---|---|---|
| 0 | `#intro` | Intro | Векторное поле контурных треугольников. Взгляд на видео влево: формируется пульсирующая Синяя таблетка. Взгляд вправо: Красная таблетка (Матрица). | Hero name, tagline, CTA кнопка |
| 1 | `#education` | Education | Концентрические кольца (Detroit style), медленно вращаются | 3 блока образования (парящий текст) |
| 2 | `#experience` | Experience | Вертикальная ось времени по центру, частицы пульсируют вдоль линии | Timeline с 2 позициями |
| 3 | `#about` | About & Stack | GSAP скролл-морфинг: треугольники плавно трансформируются в 3D-фигуры (Сфера -> Куб -> Тор) по мере скролла | Горизонтальный скролл-трек для карточек стека, bio text |
| 4 | `#services` | Services & Cost | Упорядоченная геометрическая сетка треугольников | **OptionWheel** (Центральный интерактивный контроллер услуг) + Интерактивный калькулятор сметы |
| 5 | `#contacts` | Contacts | Хаотичное облако с броуновским движением, частицы улетают к краям | Форма, соцсети, ИИ Чат-бот |
...
### 8.6 Навигационное колесо OptionWheel (Вариант Б — Центральный контроллер)

Компонент `OptionWheel` интегрируется в раздел `Services` как центральный интерактивный штурвал переключения категорий услуг и опций сметы.

*   **Конструкция**: 1px контурные тонкие круги (`opacity: 15%`), чистый `#000000` фон без карточек.
*   **Интерактивность**: Вращение колесом мыши или тач-свайпом переключает активный сектор. Активный сектор подсвечивается ярким белым/фиолетовым свечением, а связанный парящий блок с описанием и ценой обновляется на лету.

### 8.7 Интерактивный калькулятор сметы (Services)
Пошаговый конфигуратор (Landing / Web App / DevSecOps) с GSAP-счетчиком бюджета.

### 8.8 ИИ Чат-бот («Cyber Assistant»)
Плавающий Glassmorphic виджет в углу экрана, взаимодействующий с Next.js API (Gemini/OpenAI) для консультации клиентов.

### 8.9 Cookie & 152-ФЗ РФ

...
Для достижения эффекта прохождения силуэта поверх фона мы используем свойство смешивания слоев и два видео-ассета для разных тем.

### Светлая тема (Light Theme)
*   **Видео-ассет**: `/lbm.mp4` — записан на чисто белом фоне (`#ffffff`), силуэт темный.
*   **Стиль контейнера**: `mix-blend-mode: multiply` в сочетании с фоном страницы `background-color: #FAFAFA`.
...
*   **Видео-ассет**: `/lbm_dark.mp4` — записан на чисто чёрном фоне (`#000000`), тот же силуэт.
*   **Стиль контейнера**: `mix-blend-mode: screen` в сочетании с фоном страницы `background-color: #09090B`.
*   **Результат**: Чёрный цвет видео полностью исчезает, делая фон видимым, а светлый силуэт сохраняется, перекрывая точки Three.js сцены.
...
*   **Переключение тем**: При смене темы VideoInteractionService должен обновлять `src` видео-элемента и перезагружать видео, сохраняя текущую позицию воспроизведения.

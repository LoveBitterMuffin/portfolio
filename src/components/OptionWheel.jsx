...
const OptionWheel = ({
  ...
  soundUrl = '',
  soundVolume = 0.5,
  /** When true, wheel events are forwarded to the page scroll instead of
   *  being consumed by the widget — lets the sticky horizontal track work
   *  while the wheel is displayed on top of the first panel. */
  ...
  // Keep a ref so the wheel handler always reads the latest prop value
  // without needing to be recreated when it changes.
  const wheelPassthroughRef = useRef(wheelPassthrough);
  useEffect(() => { wheelPassthroughRef.current = wheelPassthrough; }, [wheelPassthrough]);
  ...
  useEffect(() => {
    ...
    if (!el) return;
    const onWheel = e => {
      // When wheelPassthrough is active, let the browser handle the event so
      // the sticky scroll-track on the page receives it normally.
      if (wheelPassthroughRef.current) {
        // Do NOT call preventDefault — bubble the event up to the scroller.
        return;
      }
      e.preventDefault();
      const cfg = cfgRef.current;
    ...
    // Must stay non-passive so we can call preventDefault in non-passthrough mode.
  ...
  const handleItemClick = useCallback(
    index => {
      ...
      onItemClickRef.current?.(index, cfgRef.current.items[index]);
      const cfg = cfgRef.current;
